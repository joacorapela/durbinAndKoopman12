...
\begin{theorem}
    ...
    \begin{alignat*}{2}
        \mathbf{x}_{t+1}&=\mathbf{u}_t+A_t\mathbf{x}_t+\mathbf{w}_t&\quad&\text{with }\mathbf{w}_t\sim N(0,Q_t)\\
        \mathbf{y}_t&=\mathbf{a}_t+B_t\mathbf{x}_t+\mathbf{v}_t&&\text{with }\mathbf{v}_t\sim N(0,R_t)\\
    ...
    \begin{align}
        \mathbf{x}_{t|t-1}&=\mathbf{u}_t+A_{t-1}\mathbf{x}_{t-1|t-1}\label{eq:xtGtm1}\\
        P_{t|t-1}&=A_{t-1}P_{t-1|t-1}A_{t-1}^\intercal+Q_{t-1}\label{eq:PtGtm1}\\
        \hat{\mathbf{y}}_{t|t-1}&\triangleq E\{\mathbf{y}_t|\mathbf{y}_1,\ldots,\mathbf{y}_{t-1}\}=\mathbf{a}_t+B_t\mathbf{x}_{t|t-1}\label{eq:ytGtm1}\\
...
\begin{proof}
    ...
    \begin{align}
        \mathbf{x}_{t|t-1}&=E\{\mathbf{x}_t|Y_{t-1}\}=E\{\mathbf{u}_t+A_{t-1}\mathbf{x}_{t-1}+\mathbf{w}_{t-1}|Y_{t-1}\}\nonumber\\
                          &=\mathbf{u}_t+A_{t-1}E\{\mathbf{x}_{t-1}|Y_{t-1}\}+E\{\mathbf{w}_{t-1}|Y_{t-1}\}\nonumber\\
                          &=\mathbf{u}_t+A_{t-1}\mathbf{x}_{t-1|t-1}+E\{\mathbf{w}_{t-1}\}\footnotemark[1]=\mathbf{u}_t+A_{t-1}\mathbf{x}_{t-1|t-1}\nonumber
    \end{align}
    ...
    \begin{align}
        \mathbf{P}_{t|t-1}&=\text{Cov}\{\mathbf{x}_t|Y_{t-1}\}=E\{(\mathbf{x}_t-\mathbf{x}_{t|t-1})(\mathbf{x}_t-\mathbf{x}_{t|t-1})^\intercal|Y_{t-1}\}\nonumber\\
                          &=E\{\left(\mathbf{u}_t+A_{t-1}\mathbf{x}_{t-1}+w_{t-1}-\mathbf{u}_t-A_{t-1}\mathbf{x}_{t-1|t-1}\right)\left(\mathbf{u}_t+A_{t-1}\mathbf{x}_{t-1}+w_{t-1}-\mathbf{u}_t-A_{t-1}\mathbf{x}_{t-1|t-1}\right)^\intercal|Y_{t-1}\}\nonumber\\
    ...
    \begin{align}
        \hat{\mathbf{y}}_{t|t-1}&=E\{\mathbf{y}_t|Y_{t-1}\}=E\{\mathbf{a}_t+B_t\mathbf{x}_t+\mathbf{v}_t|Y_{t-1}\}=\mathbf{a}_t+B_tE\{\mathbf{x}_t|Y_{t-1}\}+E\{\mathbf{v}_t|Y_{t-1}\}\nonumber\\
                                &=\mathbf{a}_t+B_t\mathbf{x}_{t|t-1}+E\{\mathbf{v}_t\}=\mathbf{a}_t+B_t\mathbf{x}_{t|t-1}\nonumber
    \end{align}
    ...
    \begin{align}
        \mathbf{z}_t&=\mathbf{y}_t-\hat{\mathbf{y}}_{t|t-1}=\mathbf{a}_t+B_t\mathbf{x}_t+\mathbf{v}_t-\mathbf{a}_t-B_t\mathbf{x}_{t|t-1}=B_t(\mathbf{x}_t-\mathbf{x}_{t|t-1})+\mathbf{v}_t\label{eq:ztProof}
    \end{align}
    ...
    \begin{align}
        p(\mathbf{x}_1)&=\mathcal{N}(\mathbf{x}_1|\mathbf{x}_{1|0},\mathbf{P}_{1|0})=\int p(\mathbf{x}_1,\mathbf{x}_0)d\mathbf{x}_0=\int p(\mathbf{x}_1|\mathbf{x}_0)p(\mathbf{x}_0)d\mathbf{x}_0\nonumber\\
                       &=\int\mathcal{N}(\mathbf{x}_1|\mathbf{u}_0+A_0\mathbf{x}_0,Q_0)\mathcal{N}(\mathbf{x}_0|\mathbf{m}_0,V_0)d\mathbf{x}_0=\mathcal{N}(\mathbf{x}_1|\mathbf{u}_0+A_0\mathbf{m}_0,A_0V_0A_0^\intercal+Q_0)\footnotemark[11]\nonumber
    \end{align}
    ...
    \begin{align*}
        \mathbf{x}_{1|0}&=\mathbf{u}_0+A_0\mathbf{m}_0\\
        \mathbf{P}_{1|0}&=A_0V_0A_0^\intercal+Q_0
    \end{align*}

    \noindent and by Eqs.~\ref{eq:xtGtm1} and~\ref{eq:PtGtm1}
